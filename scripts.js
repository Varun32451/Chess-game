...
Pawn.prototype.getValidMoves = function(n = 1) {
    let validMoves = [];
    ...
    for (let testX = 0; testX < 8; testX++) {
        for (let testY = 0; testY < 8; testY++) {
            ...
            if (toSquare) { // Check if toSquare is defined
                ...
                if (moveResult.valid) {
                    validMoves.push({ toSquare, capture: moveResult.capture, promote: moveResult.promote });
                }
            }
        }
    }
...
function squareClicked(e){
    if (selected === 1) {
        legalMoves.forEach((legalMove) => {
            const x = legalMove.toSquare.x
            const y = legalMove.toSquare.y
            getSquare(x, y).element.style.opacity = "1"
        })
    }
	...
	if(selectedSquare === null){
		if(square.piece === null){
			playSound("sounds/illegal.webm")
		}else if(square.piece.color != currentPlayer.color){
			playSound("sounds/illegal.webm")
		}else{
            selected = 1
			selectedSquare = getSquare(x, y);
            legalMoves = getSquare(x, y).piece.getValidMoves()
            legalMoves.forEach((legalMove) => {
                const x = legalMove.toSquare.x
                const y = legalMove.toSquare.y
                getSquare(x, y).element.style.opacity = "0.5"
               console.log("valid")
            })
        ...
        if(currentPlayer==white)
        {
            ...
            if(kingExposed(black.king)){
                ...
                if(isCheckmate(black.king)){
                    document.getElementById("turnInfo").innerHTML = "<b>White</b> wins!";
                    playSound("sounds/game-end.webm")
        ...
        else
        {
            ...
            if(kingExposed(white.king)){
                ...
                if(isCheckmate(white.king)){
                    document.getElementById("turnInfo").innerHTML = "<b>Black</b> wins!";
                    playSound("sounds/game-end.webm")
                    document.getElementById("board").style.pointerEvents = "none"
                    return;
                }     
...
let nextTurn = function(){
	...
	if(currentPlayer.color == "white"){
		currentPlayer = black;
        document.getElementById("turnInfo").innerHTML = "<b>Black</b> turn";
	}else{
		currentPlayer = white;
